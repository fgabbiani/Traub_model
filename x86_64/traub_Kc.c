...
extern "C" {
 ...
 static void _hoc_alpha_c(void);
 static void _hoc_beta_c(void);
 static void _hoc_rates(void);
 static void _hoc_xi(void);
...
extern Memb_func* memb_func;
 ...
 "setdata_gKc", _hoc_setdata,
 "alpha_c_gKc", _hoc_alpha_c,
 "beta_c_gKc", _hoc_beta_c,
 "rates_gKc", _hoc_rates,
 "xi_gKc", _hoc_xi,
 0, 0
};
#define alpha_c alpha_c_gKc
#define beta_c beta_c_gKc
#define xi xi_gKc
 extern double alpha_c( _threadargsprotocomma_ double );
 extern double beta_c( _threadargsprotocomma_ double );
 extern double xi( _threadargsprotocomma_ double );
...
#define _gth 0
#define c_inf_gKc _thread1data[0]
#define c_inf _thread[_gth]._pval[0]
#define tau_c_gKc _thread1data[1]
#define tau_c _thread[_gth]._pval[1]
#define usetable usetable_gKc
 double usetable = 1;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 10, 5);
 	...
 	ivoc_help("help ?1 gKc /Volumes/bcmzgabb/gabbianiz/invariance_project/Traub_model/x86_64/traub_Kc.mod\n");
...
static int _reset;
static char *modelname = "KC channel Traub";
...
double xi ( _threadargsprotocomma_ double _lcai ) {
   double _lxi;
 _lxi = _lcai / 250.0 ;
   if ( _lxi > 1.0 ) {
     _lxi = 1.0 ;
...
static void _hoc_xi(void) {
  ...
  _nt = nrn_threads;
 _r =  xi ( _p, _ppvar, _thread, _nt, *getarg(1) );
 hoc_retpushx(_r);
}
 
double alpha_c ( _threadargsprotocomma_ double _lv ) {
   double _lalpha_c;
 if ( _lv <= 50.0 ) {
     _lalpha_c = exp ( ( _lv - 10.0 ) / 11.0 - ( _lv - 6.5 ) / 27.0 ) / 18.975 ;
     }
   else {
     _lalpha_c = 2.0 * exp ( - ( _lv - 6.5 ) / 27.0 ) ;
     }
...
static void _hoc_alpha_c(void) {
  ...
  _thread = _extcall_thread;
  _nt = nrn_threads;
 _r =  alpha_c ( _p, _ppvar, _thread, _nt, *getarg(1) );
 hoc_retpushx(_r);
}
 
double beta_c ( _threadargsprotocomma_ double _lv ) {
   double _lbeta_c;
 if ( _lv <= 50.0 ) {
     _lbeta_c = 2.0 * exp ( - ( _lv - 6.5 ) / 27.0 ) - alpha_c ( _threadargscomma_ _lv ) ;
     }
   else {
     _lbeta_c = 0.0 ;
     }
...
static void _hoc_beta_c(void) {
  ...
  _thread = _extcall_thread;
  _nt = nrn_threads;
 _r =  beta_c ( _p, _ppvar, _thread, _nt, *getarg(1) );
 hoc_retpushx(_r);
}
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
 ...
 _lalpha = alpha_c ( _threadargscomma_ _lv ) ;
   _lbeta = beta_c ( _threadargscomma_ _lv ) ;
   tau_c = 1.0 / ( _lalpha + _lbeta ) ;
   c_inf = _lalpha * tau_c ;
    return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   g = gmax * c * xi ( _threadargscomma_ cai ) ;
   i = g * ( v - ek ) ;
   ik = i ;
   }
 _current += ik;
...
static void _initlists(){
 ...
 _slist1[0] = &(c) - _p;  _dlist1[0] = &(Dc) - _p;
   _t_c_inf = makevector(751*sizeof(double));
   _t_tau_c = makevector(751*sizeof(double));
_first = 0;
}
